...
Сервис формирует персональные рекомендации событий на основе гибридного подхода: **Item-based filtering** (фильтрация по схожести событий) с учетом персональных оценок пользователя.

## Входные данные

Алгоритм использует два основных источника данных:

1.  **`UserActions`** — история взаимодействий конкретного пользователя.
    *   Содержит пары: `eventId` (ID события) и `rating` (оценка пользователя).
    *   Отражает предпочтения пользователя: какие события ему понравились, а какие нет.
2.  **`EventSimilarity`** — предрассчитанная матрица схожести событий.
    *   Хранится в базе данных как связи между парами событий.
    *   Содержит коэффициент схожести (`similarity`) в диапазоне от `0.0` до `1.0`, где `1.0` означает максимальную схожесть.

## Логика работы

Алгоритм исходит из гипотезы: *«Если пользователю понравилось событие A, то с высокой вероятностью ему понравятся события, схожие с A»*.

Процесс расчета происходит в несколько этапов:

1.  **Сбор истории:** Извлекаются все действия пользователя за доступный период. Если история пуста, возвращается пустой список рекомендаций.
2.  **Поиск кандидатов:** Для каждого события из истории пользователя находятся все похожие события из матрицы схожести.
3.  **Фильтрация:** Из списка кандидатов исключаются события, с которыми пользователь уже взаимодействовал (чтобы не рекомендовать то, что он уже видел или оценивал).
4.  **Расчет веса (Scoring):** Для каждого кандидата рассчитывается итоговый балл.
5.  **Сортировка и лимитирование:** Кандидаты сортируются по убыванию балла, после чего отбирается топ-N результатов согласно параметру `maxResults`.

## Математическая модель

Итоговый балл (`Score`) для кандидата $S$ рассчитывается по формуле:

$$
Score(S) = \frac{\sum_{E \in History} (Similarity(E, S) \times Rating(E))}{Count(S)}
$$

Где:
*   $E$ — событие из истории взаимодействий пользователя.
*   $S$ — событие-кандидат на рекомендацию.
*   $Similarity(E, S)$ — коэффициент схожести между событием $E$ и кандидатом $S$ (из БД).
*   $Rating(E)$ — оценка, которую пользователь поставил событию $E$.
*   $Count(S)$ — количество раз, которое событие $S$ встретилось в списке кандидатов при переборе всей истории пользователя. Этот делитель служит для нормализации веса, чтобы события, попавшие в выборку много раз, не получали искусственно завышенный балл.

## Важные нюансы реализации

*   **Исключение просмотренного:** Пользователь никогда не получит в рекомендациях событие, с которым у него уже есть запись в `UserActions`.
*   **Нормализация:** Использование деления на `Count(S)` позволяет сгладить выбросы и сделать рекомендации более стабильными.
*   **Лимиты:** Количество возвращаемых рекомендаций ограничено параметром `maxResults`. Если параметр не задан или равен 0, возвращается до 10 событий.
*   **Производительность:** Для оптимизации запросов к базе данных используется массовая загрузка данных (`findAllByEventIdIn`) и группировка в памяти, что снижает количество обращений к БД. 